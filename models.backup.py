...
class PropCNN(nn.Module):
    ...
    def __init__(self,
        ...
        self.precomped_H = compute(slm_field,
                                   self.feature_size,
                                   self.wavelength,
                                   self.prop_dist,
        ...
        if 'none' in target_network.lower():
            ...
            self.unet_target = UnetGenerator(input_nc_target,
                                             output_nc_target,
                                             norm_layer=norm,
    ...
    def __call__(self, phase, params):
        ...
        if self.unet_target is None:
            ...
            if 'complexcnnc' in self.target_network_type:
                ...
                input_unet_target = jnp.abs(
                    target_field)  # Amplitude to target network
            elif self.target_network_type == 'stackedcnnc':
                input_unet_target = jnp.stack(
                    (jnp.real(target_field), jnp.imag(target_field)),
                    axis=1)  # (real, imag) stacked channel

            # Pad for target unet, send through unet, and then crop output
            input_unet_target = utils.pad_image(input_unet_target,
                                                self.unet_res_target)
            output_unet_target = self.unet_target.forward(
                input_unet_target, params)
            output_unet_target = utils.crop_image(output_unet_target,
                                                  self.image_res)

            if 'complexcnnc' in self.target_network_type:
                amp = jnp.abs(
                    output_unet_target)  # (real, imag) stacked channel
            elif self.target_network_type == 'cnnr':
                amp = output_unet_target  # Amplitude out of target network
            elif self.target_network_type == 'stackedcnnc':
                amp = jnp.sqrt(output_unet_target[:, 0:1, ...]**2 +
                               output_unet_target[:, 1:2, ...]**
                               2)  # Amp from (real, imag) stacked channel
